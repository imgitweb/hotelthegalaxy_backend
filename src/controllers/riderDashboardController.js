...
exports.getActiveTrip = async (req, res, next) => {
  try {
    ...
    if (!trip) {
      ...
      }).populate("user", "fullName phone").populate("items.menuItem"); 
    ...
    const activeTripOrders = trip.orderIds.filter(o => 
        isTodayOrder(o) && !["delivered", "cancelled"].includes(o.status)
    );

    if (activeTripOrders.length === 0) {
      return res.json({ success: true, data: null, message: "No active trip found" });
    }

    let orders = await Promise.all(activeTripOrders.map(o => formatOrderForRider(o)));
    orders = orders.sort((a, b) => (a.distance || Infinity) - (b.distance || Infinity));
  ...
  try {
    ...
    if (order.status === "arrived") {
      return res.status(200).json({ 
        success: true, 
        message: "Order is already marked as arrived",
...
exports.verifyDeliveryOTP = async (req, res, next) => {
  try {
    ...
    if (order.rider?.toString() !== riderId?.toString()) {
      return res.status(403).json({ success: false, message: "You are not assigned" });
    }
    ...
    // --- YAHAN CHOTA SA SAFETY FIX KIYA HAI ---
    order.status = "delivered";
    order.deliveryOTP.verifiedAt = new Date();
    order.timeline.deliveredAt = new Date();
    order.reviewSmsSent = false; // <-- SAFETY: Taki cron job ko pata chale SMS baaki hai
    await order.save();
    ...
    if (trip) {
      ...
      if (allDelivered) {
        ...
        return res.json({ success: true, message: "Order delivered successfully. Trip Completed!", tripCompleted: true });
      }
    }
...
exports.getRiderHistory = async (req, res, next) => {
  try {
    ...
    const orders = await Order.find({ rider: riderId, status: "delivered" })
      ...
      .sort({ "timeline.deliveredAt": -1, updatedAt: -1 })
      .limit(limit * 1)
      .skip((page - 1) * limit);

    const totalOrders = await Order.countDocuments({ rider: riderId, status: "delivered" });

    const formattedOrders = await Promise.all(orders.map(o => formatOrderForRider(o, false)));

    res.json({
      success: true,
      data: [{ _id: "history", orderIds: formattedOrders }],
      pagination: { currentPage: parseInt(page), totalPages: Math.ceil(totalOrders / limit), totalOrders },
    });
  } catch (error) {
    next(error);
  }
...
//     // 🛠️ IMPORTANT FIX: Sirf Pending / Out for delivery wale orders dikhayega
//     const activeTripOrders = trip.orderIds.filter(o => 
//         isTodayOrder(o) && !["delivered", "cancelled"].includes(o.status)
//     );
...
//     // 🛠️ IMPORTANT FIX: Fetch ALL 'delivered' orders directly, regardless of Trip status.
//     const orders = await Order.find({ rider: riderId, status: "delivered" })
//       .populate("user", "fullName phone")
//       .populate("items.menuItem")
//       .sort({ "timeline.deliveredAt": -1, updatedAt: -1 })
...
//     // Pseudo-trip response to match existing frontend logic
//     res.json({
//       success: true,
//       data: [{ _id: "history", orderIds: formattedOrders }],
//       pagination: { currentPage: parseInt(page), totalPages: Math.ceil(totalOrders / limit), totalOrders },
